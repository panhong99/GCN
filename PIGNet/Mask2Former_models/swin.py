...
class Window_MSA(nn.Module):
    ...
    def forward(self, x, height):
        ...
        query = rearrange(query, 'b (h p1 w p2) (n_h c) -> b (h w) n_h (p1 p2) c', h=height // self.window_size,
                          p1=self.window_size, p2=self.window_size, n_h=self.n_head)
        key = rearrange(key, 'b (h p1 w p2) (n_h c) -> b (h w) n_h (p1 p2) c', h=height // self.window_size,
                        p1=self.window_size, p2=self.window_size, n_h=self.n_head)
        value = rearrange(value, 'b (h p1 w p2) (n_h c) -> b (h w) n_h (p1 p2) c', h=height // self.window_size,
                          p1=self.window_size, p2=self.window_size, n_h=self.n_head)
        mask = rearrange(mask, 'b (h p1 w p2) -> b (h w) (p1 p2)', h=height // self.window_size, p1=self.window_size,
                         p2=self.window_size)
        ...
        result = rearrange(result, 'b (h w) n_h (p1 p2) c -> b (h p1 w p2) (n_h c)', h=height // self.window_size,
                           p1=self.window_size, p2=self.window_size, n_h=self.n_head)
    ...
    def attention(self, query, key, value, mask):
        ...
        scores = torch.matmul(query, key.transpose(-2, -1).contiguous()) / math.sqrt(
            c) + relative_position_bias.unsqueeze(0).unsqueeze(0)
        scores *= mask
...
class Swin_transformer(nn.Module):
    def __init__(self, patch_size=4, window_size=8, merge_size=2, model_dim=128, num_layers_in_stage=[2, 2, 6, 2]):
        ...
        for i, num_layers in enumerate(num_layers_in_stage):
            ...
            swin_transformer_blocks = nn.ModuleList(
                [Swin_transformer_block(channels, window_size) for _ in range(num_layers)])
    ...
    def forward(self, x):
        ...
        for i, (patch_merging, swin_transformer_blocks) in enumerate(
                zip(self.patch_merging_per_stage, self.swin_transformer_blocks_per_stage[1:])):
