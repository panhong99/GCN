...
# if False:  # segmentation (set to True to run segmentation code)
    ...
    # Pre-compute SAME/DIFF masks using per-position mode label across the batch
    mode_y_flat, valid_pos = _mode_per_position(y_in, ignore_label=ignore_label)
    # pairwise masks (P,P)
    same_pair = (mode_y_flat[:, None] == mode_y_flat[None, :]) & (valid_pos[:, None] & valid_pos[None, :])
    diff_pair = (mode_y_flat[:, None] != mode_y_flat[None, :]) & (valid_pos[:, None] & valid_pos[None, :])
    same_pair_flat = same_pair.reshape(H_dim, W_dim, H_dim, W_dim).flatten()
    diff_pair_flat = diff_pair.reshape(H_dim, W_dim, H_dim, W_dim).flatten()
    ...
    for layer_idx in range(distance_t_y.shape[0]):
        ...
        # SAME / DIFF 분리: 같은 figure에 겹쳐 그리되, 투명도와 컬러를 다르게 적용
        same_mx = mx[same_pair_flat]
        same_my = my[same_pair_flat]
        same_dist = dist[same_pair_flat]
        diff_mx = mx[diff_pair_flat]
        diff_my = my[diff_pair_flat]
        diff_dist = dist[diff_pair_flat]

        # distance는 colormap의 명암으로 표현, SAME/DIFF는 colormap 계열과 alpha로 구분
        sc_same = plt.scatter(same_mx, same_my,
                             c=same_dist, cmap='Reds',
                             alpha=0.35, s=12,
                             edgecolors='none', rasterized=True,
                             label='Same class')
        sc_diff = plt.scatter(diff_mx, diff_my,
                             c=diff_dist, cmap='Blues',
                             alpha=0.07, s=12,
                             edgecolors='none', rasterized=True,
                             label='Diff class')
