...
class GSP(nn.Module):
    def __init__(self, num_classes, depth, embedding_size, n_layer, norm=nn.BatchNorm2d, n_skip_l=1 , grid_size = 8):
        ...
        self.conv2 = nn.Conv2d(input_conv, 256, kernel_size=1, stride=1)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, multi=[1, 1, 1]):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, dilation=(dilation * multi[0]), conv=self.conv,
                            norm=self.norm))
