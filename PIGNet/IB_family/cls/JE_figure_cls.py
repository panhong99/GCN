...
def plot_scatter_combined_all_models_datasets(args, datasets_list, num,
    ...
    num_models      = max((len(d['models_list']) for d in datasets_list), default=0)
    if num_models == 0:
        print("[plot_scatter] No models found in any dataset, skipping plot.")
        return
    GAP_RATIO       = 0.30
    ...
    if fname is None:
        ...
        os.makedirs(os.path.join(FIGURE_DIR, "ALL_scatter"), exist_ok=True)
        fname  = os.path.join(FIGURE_DIR, "ALL_scatter", f"ALL_MODELS_{args.backbone}_{args.model_type}_{ds_str}_{ct_str}_scatter.png")
...
def plot_ratio_barplot_all_models_classification(args, datasets_list,
    ...
    plt.tight_layout()
    os.makedirs(os.path.join(FIGURE_DIR, "ALL_bar_plot"), exist_ok=True)
    ds_str = "_".join(ds['dataset_name'] for ds in datasets_list)
    fname  = os.path.join(FIGURE_DIR, "ALL_bar_plot", f"{ds_str}_{args.backbone}_{args.model_type}_{calcul_type}_ratio_barplot.pdf")
    plt.savefig(fname, dpi=150, bbox_inches='tight')
    plt.close()
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Scatter plot — per-model individual figures (1 × num_layers each)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def plot_scatter_per_model_datasets(args, datasets_list, num,
                                    calcul_type=None):
    """
    For each model, save a separate 1-row figure (1 × num_layers) with
    all datasets stacked horizontally — same layout as the combined figure
    but only one model row.

    Args: same as plot_scatter_combined_all_models_datasets
    """
    ct            = calcul_type or 'joint'
    ncols_scatter = max(4, num - 1)
    GAP_RATIO     = 0.30
    cols_per_ds   = ncols_scatter + 1   # scatter cols + colorbar

    def col_offset(ds_idx):
        return ds_idx * (cols_per_ds + 1)

    # Collect all model names present across datasets (preserving order)
    seen = []
    for ds_info in datasets_list:
        for m in ds_info['models_list']:
            if m['display_name'] not in seen:
                seen.append(m['display_name'])
    ...
    os.makedirs(os.path.join(FIGURE_DIR, f"model_scatter/{args.model}"), exist_ok=True)

    for model_name in seen:
        fig = plt.figure(
            figsize=((5 * ncols_scatter + 1.5) * num_datasets, 5 + 1.5),
            facecolor='white',
        )
        gs = gridspec.GridSpec(
            1, total_cols,
        ...
        for ds_idx, ds_info in enumerate(datasets_list):
            c_off        = col_offset(ds_idx)
            is_leftmost  = (ds_idx == 0)

            # Find this model in the dataset's models_list
            model_info = next(
                (m for m in ds_info['models_list'] if m['display_name'] == model_name),
                None,
            )
            if model_info is None:
                # Draw empty placeholders so layout stays consistent
                for idx in range(ncols_scatter + 1):
                    ax = fig.add_subplot(gs[0, c_off + idx])
                    ax.axis('off')
                continue
            ...
            for idx in range(num_layers, ncols_scatter):
                fig.delaxes(fig.add_subplot(gs[0, c_off + idx]))

            if last_sc is not None:
                cbar_ax = fig.add_subplot(gs[0, c_off + ncols_scatter])
                cbar    = fig.colorbar(last_sc, cax=cbar_ax)
                cbar.ax.tick_params(labelsize=FS_CBAR)
                row0_cbar_ax[ds_idx] = cbar_ax
        ...
        ds_str  = "_".join(d['dataset_name'] for d in datasets_list)
        fname   = os.path.join(
            FIGURE_DIR, f"model_scatter/{args.model}",
            f"{model_name}_{args.backbone}_{args.model_type}_{ds_str}{ct_str}_scatter.png",
        )
        plt.savefig(fname, dpi=150, bbox_inches='tight')
        plt.close()
        print(f"Per-model scatter saved: {fname}")


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Barplot — per-model individual figures
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def plot_ratio_barplot_per_model_classification(args, datasets_list,
                                                calcul_type='joint'):
    """
    For each model (excluding PIGNet_GSP), save a separate figure with
    one subplot per dataset — same style as the combined barplot but only
    that model's bars are drawn.

    Args: same as plot_ratio_barplot_all_models_classification
    """
    ct = calcul_type

    num_datasets  = len(datasets_list)

    # Collect all model names including PIGNet_GSP
    all_model_names = []
    for ds in datasets_list:
        for m in ds['models_data']:
            if m not in all_model_names:
                all_model_names.append(m)

    os.makedirs(os.path.join(FIGURE_DIR, f"model_barplot/{args.model}"), exist_ok=True)
    ds_str = "_".join(ds['dataset_name'] for ds in datasets_list)
    ...
    for model_name in all_model_names:
        ...
        # Per-model max layers (across datasets)
        model_max_layers = max(
            (len(ds['models_data'][model_name]) for ds in datasets_list
             if model_name in ds['models_data']),
            default=1,
        )
        x_base       = np.arange(1, model_max_layers + 1)
        layer_labels  = [f"B{i+1}" for i in range(model_max_layers)]

        fig, axes = plt.subplots(
            1, num_datasets,
            figsize=(max(6, model_max_layers * 1.8) * num_datasets, 5),
            facecolor='white',
        )
        if num_datasets == 1:
            axes = [axes]
        ...
        display_name = LEGEND_DISPLAY_NAMES.get(model_name, model_name)
        fig.suptitle(display_name, fontsize=35, fontweight='bold', y=1.05)

        plt.tight_layout()
        fname = os.path.join(
            FIGURE_DIR, f"model_barplot/{args.model}",
            f"{model_name}_{ds_str}_{args.backbone}_{args.model_type}_{calcul_type}_ratio_barplot.pdf",
        )
        plt.savefig(fname, dpi=150, bbox_inches='tight')
        plt.close()
        print(f"Per-model barplot saved: {fname}")
